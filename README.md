...
| | |
|---|---|
| **BusVerb / BusDelay / Send** | one aux bus: every track's AUX → a multi-mode delay → an eight-line FDN reverb → a return on track 8. A route the stock firmware has no path for. On Sam's unit. |
