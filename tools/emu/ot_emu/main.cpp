...
int main(int _argc, char** _argv)
{
	...
	bool dspPcWatchAll = false;	// keep every arrival, not the last 24
	...
	for(int i = 1; i < _argc; ++i)
	{
		...
		else if(a == "--dsp-pcwatch-all")	dspPcWatchAll = true;
	...
	if(dsp)
	{
		...
		dspPair->setPcWatchAll(dspPcWatchAll);
		...
		if(!edmaLog.empty())
		{
			...
			rtos.edma().setTransferHook([&](const uint32_t _ch, const bool _paced)
			{
				...
				std::snprintf(line, sizeof line,
					"kick ch %2u %s sample %.1f dsp0 %llu dsp1 %llu saddr %08x soff %d attr %04x nbytes %08x slast %d daddr %08x doff %d citer %04x dlast %d biter %04x csr %04x\n",
					_ch, _paced ? "paced" : "burst", rtos.sample(),
					static_cast<unsigned long long>(dspPair ? dspPair->executed(0) : 0), static_cast<unsigned long long>(dspPair ? dspPair->executed(1) : 0),
