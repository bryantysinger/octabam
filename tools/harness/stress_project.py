...
def lock_slots(track):
    # verify_set sends CC 40=100 to T2 and checks it at the end. Reserve
    # only that lane; a lock or LFO would legitimately overwrite the probe.
    return tuple(slot for slot in LOCK_SLOTS if not (track == 1 and slot == 24))
...
def mutate_bank(data, bank_number, mods):
    ...
    for part in range(otp.NPARTS_ALL):
        ...
        for track in range(8):
            ...
            data[lfo2:lfo2 + 6] = bytes((18, 19, 18 if track == 1 else 24, 1, 1, 1))
...
def main():
    ...
    report = (f"bamsep26 stress project\n"
              ...
              f"T2 SEND is reserved for verify_set MIDI probing; its third LFO targets FX1 instead.\n"
