...
Generate a local Octatrack project with eight simultaneous FLEX tracks, three active LFOs per track, 15 locked parameter slots per step (14 on T2), and the rig's costly DSP paths:
...
T2 SEND has no locks or LFO so the existing `verify_set` MIDI assertion
can hold CC 40 at 100. Its third LFO targets FX1 instead; all 24 LFOs
remain active. This is one deliberate exception to the stress workload.
