...
# The compiler from the same developer directory as the SDK below: with
# Xcode selected, the CommandLineTools swiftc (6.3.3) refused Xcode's SDK
# (built by 6.4) -- "this SDK is not supported by the compiler" (25 Sep 2026).
SWIFTC="${SWIFTC:-$(xcrun -f swiftc 2>/dev/null || echo /Library/Developer/CommandLineTools/usr/bin/swiftc)}"
