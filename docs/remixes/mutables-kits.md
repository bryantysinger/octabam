# `mutables-kits` — Five inserts + the Octakit family

The `mutables` card with Octakit, the LO-FI fix and CC PAGE 2 (bridged).
...
```bash
make image REMIX=mutables-kits BUILD=1     # -> out/OCTATRACK_OCTABAM1.bin
```

[BUILDING.md](BUILDING.md) is the walk-through from a fresh machine to a flashed unit. `make check REMIX=mutables-kits` runs every gate first.
