...
- **BusVerb** — an eight-line FDN reverb (ROOM / PLATE / BIG, shimmer, gate, mid/side width) that serves all eight tracks over a cross-core bus. Hosted on one of tracks 5–8. Knobs SEND TIME⌐SIZE SHMR⌐SHFT WET / MODE TONE DIFF GATE (⌐ = drawn as a linked pair; the 16 Sep 2026 knob pass; unflashed).
- **BusDelay** — a multi-mode delay (CLEAN / pitched GRAIN cloud / REVERSE, freeze) serving all eight tracks. Hosted on one of tracks 1–4. TIME reads as a tempo division (TEMPO SYNC); up to 739 ms (1/4 and 1/2T at 121 BPM) since the 32K lines, image 28 (15 Sep 2026, Sam: "sounds fantastic now"). Knobs SEND TIME⌐FDBK TONE PING WET / MODE SCAT⌐DENS SIZE⌐PTCH FRZE (SCAT and DENS are GRAIN's and read `---` in the other modes; unflashed).
...
- **Modulation** (FX1, on CHORUS's id) — JUNO / DIM / FLNG / COMB / PHSR, each from a permissively licensed source (`docs/effects/PORTS.md`), levelled to JUNO at the views. Knobs RATE⌐DPTH DLY FDBK MIX LOFI / MODE TONE WDTH (LOFI clocks the delay line coarse and quantises it; 0 is bit-exact). Heard on the emulator 16 Sep 2026 (ENS dropped, LOFI added); unflashed.
