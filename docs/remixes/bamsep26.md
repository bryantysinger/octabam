...
- **Character** (FX1, on LO-FI's id) — crush, fold/ring, saturation, compressor, width; GLUE compression on track 8 by position. Knobs DRV FOLD TXTR COMP — MIX / SAT TONE WDTH (16 Sep 2026: MIX bottom right on page 1; SAT top left on page 2, where every effect's MODE sits; page-1 slot 4 was the bus return until 20 Sep 2026 and is empty).
...
- After flashing, stamp every project you will play before pressing play: `python3 tools/hw/ot_project.py stamp-defaults <project> bamsep26 --all --keep-mode` (the 16 Sep 2026 knob pass moved slots on BusVerb, Character and Modulation; 20 Sep 2026 emptied Character's slot 4). A part saved under another layout feeds the stations its old bytes and the sequencer stalls. The stamp warns about an engine on the wrong core (BusVerb on T1–4, BusDelay on T5–8: it runs as SEND there).
