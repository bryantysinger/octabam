...
- **Modulation** (FX1, on CHORUS's id) — JUNO / DIM / FLNG / COMB / PHSR, each from a permissively licensed source (`docs/effects/PORTS.md`), levelled to JUNO at the views. Knobs RATE⌐DPTH DLY FDBK LOFI MIX / MODE TONE WDTH (MODE top left, as on every effect; COMB names RATE DPTH WDTH `---`, PHSR names TONE `---`) (LOFI clocks the delay line coarse and quantises it; 0 is bit-exact). Heard on the emulator 16 Sep 2026 (ENS dropped, LOFI added); unflashed.
