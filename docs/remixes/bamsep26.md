...
On Sam's MKII as image 38 (`OCTABAM38`, 20 Sep 2026: the engines print their wet on their hosts, the glides, the wow; `CHANGELOG.md` per image). Unflashed on main: the TIME ramp within the block, the `---` names per mode, the names per mode with Character's TONE on page 1 (slot 4) and WDTH on page-2 slot 7. Modulation's five modes have not been heard on the unit. The one-aux bus claims all pass on hardware (flash 7); the sends into the delay with the note-only tempo cave, image 24. Every other stock effect is harvested: 13 effects; a saved part naming one plays silence.
